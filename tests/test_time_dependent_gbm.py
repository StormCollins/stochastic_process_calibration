...
def test_get_time_dependent_volatility():
    ...
    gbm: TimeDependentGBM = TimeDependentGBM(drift, excel_file_path, 'constant_vol_surface')
    np.random.seed(999)
...
def test_get_time_dependent_gbm_paths_for_constant_vols():
    ...
    time_to_maturity = 1
    time_dependent_gbm: TimeDependentGBM = TimeDependentGBM(drift, excel_file_path, 'constant_vol_surface')
    np.random.seed(999)
    actual_paths = \
    ...
    time_independent_gbm: TimeIndependentGBM = TimeIndependentGBM(drift, volatility)
    np.random.seed(999)
    expected_paths = \
